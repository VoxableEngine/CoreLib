...
function Entity:add(component)
    ...
    if self.components[name] then
        ECS.Debug("Entity: Trying to add Component '" .. name .. "', but it's already existing. Please use Entity:set to overwrite a component in an entity.")
    else
...
function Entity:remove(name)
    if self.components[name] then
        ...
        ECS.Debug("Entity: Trying to remove non-existent component " .. name .. " from Entity. Please fix this")
    end
    --if self.eventManager then
